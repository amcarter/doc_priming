...
setwd("C:/Users/julia/Dropbox/Kirby_priming/Data_Processing")

# data ####
# read in sample metadata file for the entire duration of experiment
dat <- read_csv("Kirby_trial_run_all_2023/Dataharvest_weekof_07_03")

#metadata file with syringes times and dates and treatments, and alkalinity (assumed) ^^
#check files from github
...
# filelist <- list.files('Kirby_trial_run_all_2023')
# t = 0

dd <- mutate(dat,
              rundate = as.Date(DATETIME),
              pressure = 0.9)
# for(f in filelist){ # compile all processed data from experiment into one table
# 
#     dd <- read_csv(paste0("Kirby_trial_run_all_2023/", f)) %>%
...
# dat$sample_interval <- c(NA, 1,1,2,2)
#     
# dat <- dat %>%
...
dat %>% slice(-1, -2, -3, -4, -6, -7) %>%
ggplot(aes(x = DATETIME, y = CO2_conc_umolL))+
    geom_point()

##CONVERTING TO gCO2##

dat$mgC <- dat$CO2_conc_umolL*(12010/1000000)
dat$CO2_13 <- dat$mgC*deltoAF(dat$delCO2)

dat %>% slice(-1, -2, -3, -4, -6, -7) %>%
  ggplot(aes(x = DATETIME, y = mgC))+
  geom_point()

dat %>% slice(-1, -2, -3, -4, -6, -7) %>%
  ggplot(aes(x = DATETIME, y = CO2_13))+
  geom_point()

dat %>% slice(-1, -2, -3, -4, -6, -7) %>%
  ggplot(aes(x = DATETIME, y = delCO2))+
  geom_point()

mgCdiff <- ((dat$mgC[12]+dat$mgC[13])/2) - dat$mgC[5]
mgCday <- mgCdiff/8
