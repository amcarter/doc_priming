...
# setup ####
dat <- read_csv("data/Kirby_NyackW/nw_expt.csv")
...
p13 <- ggplot(dat_dif, aes(sample_datetime, DIC_13C_ugL, col = trt, lty = factor(leachate)))+
    ...
    xlab('Date') +theme_bw()
png(filename = 'figures/nw_expt_13CO2.png',
...
png(filename = 'figures/nw_expt_13CDIC.png',
...
ggplot(excess13C, aes(carbon, excess_13C_DIC, col = nuts, group = nuts)) +
    ...
    ggtitle('Nyack Well HA-7')
dev.off()

png(filename = 'figures/nw_expt_13CDIC_pres.png',
    width = 5, height = 5, res = 300, units = 'in')
